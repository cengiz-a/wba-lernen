...
function drawForeground(ctx, parameters) {
 ...
 ctx.arc(parameters.centerX,parameters.centerY,parameters.hourRadius,parameters.startAngle,parameters.endAngleHour);
    ...
    //Minuten-Ring
    ctx.beginPath();
    ctx.arc(parameters.centerX,parameters.centerY,
            parameters.minuteRadius,parameters.startAngle,parameters.endAngleMinute);
    ctx.stroke();
    ctx.closePath();
    ...
    ctx.beginPath();
    ctx.arc(parameters.centerX,parameters.centerY,
            parameters.secondRadius,parameters.startAngle,parameters.endAngleSecond);
    ctx.stroke();
    ctx.closePath();

    ctx.font = "30px Arial";
    ctx.fillText(now.getHours()%12,220,250);
    ctx.fillText(now.getMinutes(),230,250);
    ctx.shadowBlur = 0;
